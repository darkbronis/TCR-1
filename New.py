...
➤  Ban
➤  Unban
➤  .personalbc [ Contact BroadCast ]
➤  .groupbc [ Group BroadCast ]
➤  .spm [ .spm 5 Test ]
➤  Mid @
➤  Bye @
➤  bye @
...
wait = {
    ...
    'timeline':True,
    'autoAdd':True,
    'message':"Owner : line://ti/p/~khalik02",
    "lang":"JP",
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
					if msg.toType == 2:
						...
						kk.sendText(msg.to,"Bawa enjoy aja")
						targets = []
						for g in gs.members:
...
#-------------------------------------------------------------
            ...
            elif ".personalbc " in msg.text:
               if msg.from_ in admin:
                 bctxt = msg.text.replace(".personalbc ", "")
...
#--------------------------------- TRANSLATE --------------------------------
                ...
                try:
                    ...
                    cl.sendText(msg.to,'Error lu bau')
...
#-----------------------------------------------
            ...
            elif "Adminrm @" in msg.text:
                if msg.from_ in creator:
                    print "[Command]Staff remove executing"
                    _name = msg.text.replace("Adminrm @","")
                    _nametarget = _name.rstrip('  ')
                    gs = cl.getGroup(msg.to)
                    gs = ki.getGroup(msg.to)
                    gs = kk.getGroup(msg.to)
...
#-----------------------------------------------
            elif msg.text in ["Lockcancel:on","lockcancel:on"]:
              if msg.from_ in admin:
                 if wait["Protectcancel"] == True:
                     if wait["lang"] == "JP":
                         cl.sendText(msg.to,"Jangan cancel siapa pun, awas nanti terkick")
                     else:
                         cl.sendText(msg.to,"done")
                 else:
                    ...
                    if wait["lang"] == "JP":
                        ...
                        cl.sendText(msg.to,"done")
            elif msg.text in ["Lockcancel:off","lockcancel:off"]:
              if msg.from_ in admin:
...
#-----------------------------------------------
				...
				if msg.from_ in admin:
					cl.sendText(msg.to,"Hadir Aizen-sama ")
#-------------------------------------------------
...
#---------------------------------------
            elif ".googlesay " in msg.text.lower():
                    query = msg.text.replace(".googlesay ","")
                    with requests.session() as s:
                        s.headers['user-agent'] = 'Mozilla/5.0'
...
#------------------------------------------------------------------
               ...
               if msg.from_ in admin:
                wait["blacklist"] = {}
                cl.sendText(msg.to,"clear ban success") 
     ...
     for zx in range(0,100):
        ...
        if hasil['result']['posts'][zx]['postInfo']['liked'] == False:
          try:    
            ...
            cl.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"Autolike By Rakha ")
            #-----------------------------[JANGAN DIEDIT - Hargai Saya]-----------------------------#
            cl.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"Auto Like By Rakha")
            print "Like"
          except:
            pass
        else:
